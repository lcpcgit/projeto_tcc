        ...
        if modo_visao == "Visão Geral":
            ...
            if familia_input:
                ...
                if not df_filtrado.empty:
                    ...
                    # ✨ NOME DO PRODUTO (FAMÍLIA) NO HOVER
                    fig.update_traces(
                        textposition="top center",
                        hovertemplate=f"<b>Produto:</b> {familia_input.upper()}<br><b>Data da Extração:</b> %{{x|%d/%m/%Y}}<br><b>Média:</b> R$ %{{y:,.2f}}<extra></extra>"
                    )
            ...
            if produto_escolhido and not esta_bloqueado:
                ...
                if not df_filtrado.empty:
                    ...
                    # ✨ NOME DO PRODUTO ESPECÍFICO NO HOVER
                    fig.update_traces(
                        textposition="top center",
            ...
            if not df_drill.empty:
                ...
                with col_filtro_preco:
                    filtro_preco = st.selectbox(
                        "Faixa de Preço",
                        [
                            "Todos os Preços", "Abaixo de R$ 100", "R$ 100 a R$ 500", 
                            "R$ 500 a R$ 1.500", "R$ 1.500 a R$ 3.000", 
                            "R$ 3.000 a R$ 5.000", "R$ 5.000 a R$ 8.000", "Acima de R$ 8.000"
                        ]
                    )
                
                if filtro_preco == "Abaixo de R$ 100": df_drill = df_drill[df_drill['Preco'] < 100]
                elif filtro_preco == "R$ 100 a R$ 500": df_drill = df_drill[(df_drill['Preco'] >= 100) & (df_drill['Preco'] <= 500)]
                elif filtro_preco == "R$ 500 a R$ 1.500": df_drill = df_drill[(df_drill['Preco'] > 500) & (df_drill['Preco'] <= 1500)]
                elif filtro_preco == "R$ 1.500 a R$ 3.000": df_drill = df_drill[(df_drill['Preco'] > 1500) & (df_drill['Preco'] <= 3000)]
                elif filtro_preco == "R$ 3.000 a R$ 5.000": df_drill = df_drill[(df_drill['Preco'] > 3000) & (df_drill['Preco'] <= 5000)]
                elif filtro_preco == "R$ 5.000 a R$ 8.000": df_drill = df_drill[(df_drill['Preco'] > 5000) & (df_drill['Preco'] <= 8000)]
                elif filtro_preco == "Acima de R$ 8.000": df_drill = df_drill[df_drill['Preco'] > 8000]

                if not df_drill.empty:
                    # 🚀 AQUI ESTÁ A MÁGICA: Sempre agrupamos pelo nome exato do Produto para não perdê-lo!
                    df_agrupado_drill = df_drill.groupby(['DataCaptura', 'Loja', 'Marca', 'Produto'])['Preco'].mean().reset_index()
                    
                    # Cria uma Legenda encurtada para não quebrar a tela
                    def formatar_legenda(row):
                        marca = str(row['Marca']).upper()
                        prod = str(row['Produto'])
                        if len(prod) > 35:
                            return f"[{marca}] {prod[:35]}..."
                        return f"[{marca}] {prod}"

                    df_agrupado_drill['Legenda'] = df_agrupado_drill.apply(formatar_legenda, axis=1)
                    df_agrupado_drill['Preco_Label'] = df_agrupado_drill['Preco'].apply(lambda x: f"R$ {x:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
                    ...
                    for loja in lojas_presentes:
                        ...
                        if not df_loja.empty:
                            fig_drill = px.line(
                                ...
                                markers=True, 
                                text="Preco_Label", 
                                custom_data=['Produto'], # 🚀 Puxa o nome inteiro secreto para usar na caixa preta
                                title=f"🏢 {loja.upper()} | {titulo_graf} {subtitulo}", 
                                labels={"DataCaptura": "Data da Extração", "Preco": "Preço Médio (R$)", "Legenda": "Item"}
                            )
                            
                            # ✨ HOVER PERFEITO COM O NOME EXATO DO PRODUTO
                            fig_drill.update_traces(
                                textposition="top center",
                                hovertemplate="<b>Produto:</b> %{customdata[0]}<br><b>Data da Extração:</b> %{x|%d/%m/%Y}<br><b>Média:</b> R$ %{y:,.2f}<extra></extra>"
                            )
